...
const AdminLayout = () => {
  ...
  }, [canEditDishes, canManageAvailability, canDeleteDishes, canManageUsers, activeTab]);

  const hasDishPermissions = canEditDishes || canManageAvailability || canDeleteDishes;
  const hasUserPermissions = canManageUsers;

  // Si no tiene ningún permiso, no debería llegar aquí, pero por seguridad
  if (!hasDishPermissions && !hasUserPermissions) {
    return (
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-background to-warm-cream/30">
      <div className="container mx-auto px-4 py-8">
        ...
        {/* Solo mostrar tabs si hay más de una pestaña disponible */}
        {hasDishPermissions && hasUserPermissions ? (
          ...
          <div className="w-full">
            ...
            {hasUserPermissions && <UserManagement />}
          </div>
        )}
      </div>
    </div>
